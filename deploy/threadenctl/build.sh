...
build_web() {
  ...
  if node_version_ok && command_exists npm; then
    runuser -u threaden -- env HOME="$BUILD_CACHE/home" npm --prefix "$src" ci --no-audit --no-fund
    ...
    docker run --rm --user "$(id -u threaden):$(id -g threaden)" -e HOME=/tmp \
      -v "$src:/src" -w /src "$NODE_IMAGE" sh -ceu 'npm ci --no-audit --no-fund && npm run build'
  fi
