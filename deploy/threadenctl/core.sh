...
validate_env_file() {
  ...
  grep -Eqi 'replace-with|substituteme|example\.(com|org|net)' "$PROJECT_ENV" && die "$PROJECT_ENV contains placeholders"
  local key value
...
validate_livekit_config() {
  ...
  grep -Eqi 'replace-with|substituteme|example\.(com|org|net)' "$PROJECT_LIVEKIT_CONFIG" && die "$PROJECT_LIVEKIT_CONFIG contains placeholders"
