...
Публичная web-версия управляется отдельным unit через `--public` и слушает
`127.0.0.1:18082`. Внешний nginx должен проксировать публичный домен на этот
порт. Локальная web-версия использует `--web` и порт `127.0.0.1:18081`.
