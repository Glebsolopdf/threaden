...
5. **Password policy.** Новая регистрация принимает пароли длиной 6–72 символа (не более 72 UTF-8 байт); вход сохраняет совместимость со старыми аккаунтами, но ограничивает пароль 72 байтами.
...
- До миграции `npm run build` проходил для прежнего Vite-клиента. Для Angular-версии выполнена синтаксическая проверка всех TypeScript-файлов; полноценный `ng build` в этой среде не запускался, потому что npm registry не отвечал при установке зависимостей.
