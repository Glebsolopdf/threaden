      ...
      @for (attachment of attachments(); track attachment.id) {
        @if (attachment.kind === 'image') {
          ...
          <a class="message-attachment" [href]="attachment.url" [attr.download]="attachment.name">
            <span class="message-attachment__file"><span aria-hidden="true">⇩</span><span class="message-attachment__meta"><strong>{{ attachment.name }}</strong><small>{{ formatBytes(attachment.size) }} · {{ attachment.kind === 'archive' ? 'Архив' : attachment.mime }}</small></span></span>
          </a>
        }
      }
    </div>
