...
func (s *Store) CreateUserWithSession(
	...
	if _, err = tx.ExecContext(ctx, `
		...
		VALUES(?, ?, ?, ?, ?)`, tokenHash[:], user.ID, user.CreatedAt.Unix(), user.CreatedAt.Unix(), expiresAt.Unix()); err != nil {
		if isConstraint(err) {
			return ErrConflict
		}
...
func (s *Store) DeleteInactiveUsers(ctx context.Context, cutoff time.Time) error {
	if _, err := s.db.ExecContext(ctx, `DELETE FROM users WHERE last_seen_at < ?`, cutoff.Unix()); err != nil {
		return fmt.Errorf("delete inactive users: %w", err)
	}
	return nil
}
