...
func newAPI(t *testing.T, maxMembers int) *testAPI {
	...
	server := httptest.NewServer(httpapi.NewWithSecurity(service, groupService, st, logger, []string{"https://client.test"}, security))
	t.Cleanup(func() {
		server.Close()
		st.Close()
	})
...
func TestAPILifecycleValidationAndCORS(t *testing.T) {
	...
	if status != http.StatusUnauthorized || !bytes.Contains(body, []byte(`"code":"invalid_credentials"`)) {
