...
func DefaultConfig() Config {
	return Config{
		...
		IPBanThreshold:          10,
		IPBanWindow:             15 * time.Minute,
		IPBanSteps:              []time.Duration{10 * time.Second, time.Minute, 5 * time.Minute, 24 * time.Hour},
		IPBanEscalationForget:   24 * time.Hour,
		AccountBanWindow:        30 * 24 * time.Hour,
		AccountBanDeletionCount: 5,
	}
}
