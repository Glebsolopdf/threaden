...
| `ATTACHMENT_MAX_USER_DAILY_BYTES` | Суточный лимит новых аккаунтов; после первых 24 часов лимит отключается |
...
Для не-локального `LIVEKIT_PUBLIC_URL` backend не запустится с демонстрационной
парой `LIVEKIT_API_KEY=devkey` и `LIVEKIT_API_SECRET=secret`: задайте уникальные
production-ключ и секрет.
...
Раздел аккаунта «Квоты» использует защищённые endpoint:

| Метод | Endpoint | Назначение |
|---|---|---|
| `GET` | `/v1/account/quotas` | текущая квота, лимиты и состояние удаления |
| `POST` | `/v1/account/attachments/delete-all` | запланировать удаление сообщений с вложениями через 5 минут |
| `DELETE` | `/v1/account/attachments/delete-all` | отменить ожидающее удаление |

Плановое удаление переживает перезапуск сервера. Оно удаляет целиком сообщения
пользователя, содержащие его вложения, включая подписи; до начала выполнения его
можно отменить.
...
проверки перекодируются до 1 МБ, аудио сохраняется без перекодирования после
проверки содержимого, архивы принимаются по сигнатуре до 5 МБ и не
распаковываются. Поддерживаются WebM/Opus, OGG/Opus, MP3, WAV и M4A/MP4;
браузерная запись ограничена пятью минутами. Активная квота пользователя — 50
МБ, срок хранения всех вложений — 72 часа.
